...
def check_if_gamer(gamer):
    if gamer.get('name') and gamer.get('availability'):
        return True
    return False
def add_gamer(gamer, gamers_list):
    if check_if_gamer(gamer):
...
def find_best_night(availability_table):
    best_night = ''
    for day,no_of_gamers in availability_table:
        if no_of_gamers > availability_table.get(best_night,0):
            best_night = day
    return best_night
...
def find_best_night(availability_table):
    best_night = ''
    for day,no_of_gamers in availability_table.items():
        if no_of_gamers > availability_table.get(best_night,0):
            best_night = day
    return best_night
...
def available_on_night(gamers_list,day):
    # returns a *list of names* of available gamers 
    return [gamer['name'] for gamer in gamers_list if day in gamer['availability']]

def gamers_not_available_on_night(gamers_list,day):
    # returns a *list of gamers* not available
...
form_email = """Dear {name},\nWe are happy to inform you that you have been invited to attend game night on {day_of_week} for the game of {game}."""
...
def gamers_available_on_night(gamers_list,day):
    # returns *list of gamers* available on a night (not printing out)
    return [gamer for gamer in gamers_list if day in gamer['availability']]

unable_to_attend_best_night = gamers_not_available_on_night(gamers,game_night)
...
available_second_game_night = available_on_night(gamers,game_night)
